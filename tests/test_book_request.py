...
def test_get_book_request(client, db, book, book_request):
    # test 404
    rep = client.get(
        "/api/v1/request/100000", headers={"content-type": "application/json"}
    )
    ...
    # test 200
    rep = client.get(
        "/api/v1/request/%d" % book_request.id,
...
def test_del_book_request(client, db, book, book_request):
    ...
    rep = client.get("/api/v1/request/1", headers={"content-type": "application/json"})
    assert rep.status_code == 200

    # pouf!
    rep = client.delete(
        "/api/v1/request/1", headers={"content-type": "application/json"}
    )
    assert rep.status_code == 200
...
def test_create_book_request(client, db, book):
    ...
    rep = client.post(
        "/api/v1/request", json=data, headers={"content-type": "application/json"}
    )
    ...
    data = {"title": "Fascinating stories volume:1", "email": "me@myself.com"}
    ...
    data = {"title": book.title, "email": "me@myself.com"}
...
def test_get_all_book_requests(client, db, book, book_request):
    ...
    db.session.commit()
    book_requests = [
        BookRequest(email="user_%d" % i, book_id=book.id) for i in range(5)
    ]
    db.session.add_all(book_requests)
    db.session.commit()

    rep = client.get("/api/v1/request", headers={"content-type": "application/json"})
    assert rep.status_code == 200
